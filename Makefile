            ...
            build-essential autoconf automake libxmu-dev \
             python3-pygraphviz cvs mercurial bzr git cmake \
              python3-matplotlib python-tk python3-dev qt5-qmake \
               gnuplot-x11 p7zip-full gir1.2-goocanvas-2.0 virtualenv git\
...
run:
	make copy
	cd ns-allinone-$(VERSION)/ns-$(VERSION)/ && ./waf -v --run 2DTrilateration
vis-1d:
	make copy
	cd ns-allinone-$(VERSION)/ns-$(VERSION)/ && ./waf --run 1DDistanceProfiling --vis
...
graph-1d:
	...
	env/bin/python rssi-filter-profiling/profiler.py --verbose true --file ns-allinone-$(VERSION)/ns-$(VERSION)/capture_1.csv \

graph-2d:
	make copy
	cd ns-allinone-$(VERSION)/ns-$(VERSION)/ && ./waf --run 2DTrilateration
